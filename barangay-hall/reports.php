...
try {
    ...
    $stmt = $conn->query("SELECT DATE_FORMAT(created_at, '%Y-%m') as month, COUNT(*) as count FROM applications GROUP BY DATE_FORMAT(created_at, '%Y-%m') ORDER BY month DESC LIMIT 12");
    $stats['monthly_applications'] = $stmt->fetchAll();
    
    $stmt = $conn->query("SELECT DATE_FORMAT(created_at, '%Y-%m') as month, COUNT(*) as count FROM community_concerns GROUP BY DATE_FORMAT(created_at, '%Y-%m') ORDER BY month DESC LIMIT 12");
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'export_applications':
                try {
                    $stmt = $conn->query("
                        SELECT a.id, a.service_type, a.service_id, a.status, a.application_date, a.created_at, a.remarks,
                               u.full_name, u.email,
                    ...
                    $output = fopen('php://output', 'w');
                    fputcsv($output, ['ID', 'Service Type', 'Service Name', 'Status', 'Applicant', 'Email', 'Application Date', 'Remarks']);
