...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'register_patient') {
    ...
    if (empty($blood_type)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT id FROM patient_registrations WHERE user_id = ? AND status IN ('pending', 'approved')");
            $stmt->execute([$user_id]);
            if ($stmt->fetch()) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                if (empty($error)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO applications (user_id, service_type, service_id, purpose, requirements_files, status) VALUES (?, ?, ?, ?, ?, 'pending')");
                    $stmt->execute([$user_id, $service_type, $service_id, $purpose, $uploaded_files_json]);
                                    ...
                                    <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-red-700 mt-1">Your patient registration was not approved. Please contact the health center for more information.</p>
                                            </div>
                                        </div>
                                    </div>
                                <?php endif; ?>
