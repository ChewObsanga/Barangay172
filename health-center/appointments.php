...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['CONTENT_TYPE'] ?? '', 'application/json') !== false) {
    ...
    if ($data && isset($data['action']) && $data['action'] === 'confirm_appointment') {
        try {
            // Check if user is logged in and is health_staff for this API call
            ...
            $staff_check_stmt = $conn->prepare("SELECT id FROM users WHERE id = ? AND role IN ('health_staff', 'admin')");
...
// Check if user is logged in and is health_staff (for regular page access)
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'health_staff') {
        ...
        <div id="sidebar" class="w-64 bg-white shadow-lg min-h-screen transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out fixed lg:relative z-40">
            <div class="p-4">
                <nav class="space-y-2">
                    ...
                    <!-- Medical Records -->
                    <a href="medical-records.php" class="flex items-center px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <?php foreach ($appointments as $appointment): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div class="text-sm text-gray-500"><?php echo date('H:i A', strtotime($appointment['appointment_date'])); ?></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php
                                                                        $status_colors = [
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                ...
                                                <form method="POST" class="inline">
                                                    ...
                                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this appointment? This action cannot be undone.')" 
